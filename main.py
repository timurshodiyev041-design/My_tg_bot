...
def preload_rates():
    try:
        r = requests.get("https://api.frankfurter.app/latest", timeout=5)
        data = r.json()
        data["rates"]["EUR"] = 1.0
        cache["rates"] = data["rates"]
        cache["timestamp"] = time.time()
        cache["ok"] = True
        print("✅ Курсы валют обновлены", time.strftime("%H:%M:%S"))
    except Exception as e:
        print("⚠️ Не удалось обновить курсы:", e)
        cache["ok"] = False

# --- Фоновое обновление курсов каждые 10 минут ---
